...
function Row({ label, value, valueClass = 'text-slate-800 dark:text-slate-200', tip, bold }) {
  return (
    <div className="flex items-center justify-between py-1.5">
      <div className="flex items-center gap-1.5">
        <span className={`text-sm ${bold ? 'font-semibold text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>{label}</span>
...
export default function TaxSavingsPlanner({ transactions = [], calculation, employmentType }) {
  ...
  const estimates = useMemo(() => {
    ...
    const businessExpenses = (employmentType === 'self_employed' || employmentType === 'business_owner')
      ? transactions
          .filter(t => !t.is_income && (t.business_flag === 'business' || t.business_flag === 'mixed') && t.deductible_flag)
          .reduce((s, t) => {
  ...
  }, [transactions, calculation, taxPaid, useProjected, monthsElapsed, remainingMonths]);
      ...
      <CardContent className="space-y-4">
        ...
        {noTaxDue ? (
          <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
            ...
            <p className="text-sm text-emerald-700 dark:text-emerald-400">You have no outstanding estimated tax liability.</p>
          </div>
        ) : (
          <div className="space-y-0.5">
            ...
            />
            <Row
              label="Tax Paid to Date"
              value={fmt(Number(taxPaid) || 0)}
            ...
            <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
              ...
              <span className="text-lg font-bold text-emerald-700 dark:text-emerald-300 tabular-nums">{fmt(estimates.monthlyReserve)}</span>
            </div>
          </div>
        )}
        ...
        {!noTaxDue && (
          <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-2">
              {reminderEnabled ? <Bell className="w-4 h-4 text-emerald-500" /> : <BellOff className="w-4 h-4 text-slate-400" />}
              <div>
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 pt-1 leading-relaxed border-t border-slate-100 dark:border-slate-700 pt-3">
          This is an estimate based on recorded income and deductible expenses. Actual tax payable depends on official filing and tax authority assessment.
        </p>
      </CardContent>
    </Card>
  );
