...
const navigation = [
  ...
  { name: 'Tax Calculator', icon: Calculator, page: 'TaxCalculator' },
...
export default function Sidebar({ currentPage, collapsed, onToggle, subscription, isAdmin }) {
  const handleLogout = () => {
    base44.auth.logout('/');
  };
  ...
  return (
    <aside className={cn(
      "fixed left-0 top-0 h-full bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transition-all duration-300 z-40 flex flex-col",
      collapsed ? "w-16" : "w-64"
    )}>
      {/* Logo */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200 dark:border-slate-700">
        ...
        <Button variant="ghost" size="icon" onClick={onToggle} className="text-slate-400">
