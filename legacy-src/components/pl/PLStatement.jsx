...
import PLSummaryRow from './PLSummaryRow';
import { getCategoryLabel } from './usePLData';

const fmt = (n) => `₦${Math.abs(n).toLocaleString()}`;

export default function PLStatement({ data, periodLabel }) {
  const {
  ...
  return (
    <Card className="bg-white dark:bg-slate-800 border-0 shadow-sm">
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        ...
        <Badge variant="outline" className="text-slate-500 text-xs">{periodLabel}</Badge>
      </CardHeader>
      <CardContent className="space-y-1">

        {/* INCOME */}
        <p className="text-xs font-bold uppercase tracking-wider text-slate-400 pt-2 pb-1">Business Income</p>
        ...
        {/* EXPENSES */}
        <p className="text-xs font-bold uppercase tracking-wider text-slate-400 pt-4 pb-1">Business Expenses</p>
